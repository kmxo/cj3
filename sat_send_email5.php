...
echo "<br>$mensagem";
    ...
    foreach ($db->query($sql) as $row) {
        ...
        try {
            ...
            //---------------------------------------- Send email to @gmail  ------------------------------------------------------------
            try {
                $mail->addAddress('carlos.camacho.talk@gmail.com');
            } catch (Exception $e) {
                $msg = "\nInvalid address skipped: @gmail ";
                logMe($msg);
            }
        
            try {
                $mail->send();
                $msg = "\nMessage sent to: carlos.camacho.talk@gmail.com";
                logMe($msg);
                
            } catch (Exception $e) {
                $msg = 'Mailer Error gmail ' . $mail->ErrorInfo;
                logMe($msg);
                //Reset the connection to abort sending this message
                //The loop will continue trying to send to the rest of the list
                $mail->getSMTPInstance()->reset();
    ...
    $mensagem = "\n$fim - Fim de Execucao sat_send_email.php\n--------------------------------------------------------------";
    logMe($mensagem);

    echo "<br>$fim - Fim de execucao sat_send_email.php";
